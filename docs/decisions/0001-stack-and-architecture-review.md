...
| **Styling**        | Tailwind CSS v4 (`@tailwindcss/vite`), palette as `@theme` tokens                      | Requested by the project owner. Originally CSS Modules was proposed (zero dependency), but Tailwind is a reasonable fit: utilities are generated from the palette tokens, so colors stay centralized, and UI primitives stay small. Cost: one dependency and class-string verbosity; mitigated by keeping repeated patterns inside `components/ui`. |
...
| **Not added**      | Zustand/Redux, TanStack Query Devtools, `web-vitals` (for now), `react-error-boundary` | No shared client state that justifies a store (Lab state is a tiny `useSyncExternalStore` module). `web-vitals` is decided when the performance step starts.                                                                                                                                                                                        |
...
- CSS Modules + custom properties: no dependency, but more boilerplate per component (initially proposed; replaced by Tailwind on request).
